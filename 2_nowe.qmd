...
## Zadanie 3
 ...
 mam zdania", "zgadzamsię", "zdecydowanie się zgadzam" na pytanie PYT_1. Przyjmij poziom
...
```
P-value mniejsze od poziomu istotności, więc odrzucamy hipotezę $H_0$ o niezależności zmiennych.
...
## Zadanie 7 i 8
Zapoznaj się z funkcją służącą do wykonania testu niezależności chi-kwadrat i zweryfikuj hipotezę, że stopień zadowolenia ze szkoleń w kontekście dopasowania do indywidualnych potrzeb w pierwszym
 badanym okresie nie zależy od zajmowanego stanowiska. Przyjmij poziom istotności 0.01.
 Wynik testu porównaj z wynikiem uzyskanym w zadaniu 6. Zaprezentuj reszty wyznaczane w
 teście na wykresie asocjacyjnym i dokonaj jego interpretacji.
...
```
**Wnioski**
Dla większych $n$ moc testu drastycznie rośnie.
...
poziom_kretyczny(ankieta$PYT_2, ankieta$CZY_KIER)
```
# Część IV i V
...
Przeprowadzone wsród brytyjskich mężczyzn badanie trwające 20 lat wykazało, że
 ...
 silniejszy?
```{r, echo = FALSE, warning = FALSE, message = FALSE}
x <- matrix(c(0.00140, 0.00010, 0.00669,0.00413), nrow =2, byrow = FALSE, dimnames = list(PALI = c("Tak", "Nie"), POWÓD = c("Rak", "Serce")))
x
```
...
analiza_korespondencji <- function(zmienna1, zmienna2){
  ...
  plot(row_stdx, row_stdy, col = "blue", pch = 16, xlab = "Współrzędna X", ylab = "Współrzędna Y", main = "Analiza korespondencji",  ylim = c(-4, 2.5), xlim=c(-1,9))
  points(col_stdx, col_stdy, col = "red", pch = 16)
  legend("topright", legend = c("PYT_2", "STAŻ"), col = c("blue", "red"), pch = 16)
  abline(h = 0, col = "black", lty = 2)
  abline(v = 0, col = "black", lty = 2)
  text(row_stdx, row_stdy, labels = as.character(c(-2,-1,1,2)), pos = 3, col = "blue")  # Odpowiedzi wierszy
  text(col_stdx, col_stdy, labels = as.character(c(1,2,3)), pos = 3, col = "red")  # Odpowiedzi kolumn
}
